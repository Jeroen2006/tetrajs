...
class TetraJS {
    ...
    constructor(serialPort, baudRate, options) {
        ...
        this.#serialPort.write('AT+CTSP=2,0,0\r\n')     //Call Signaling
        ...
        this.#serialPort.write('AT+CTSDS=12,0\r\n');  
        ...
        this.#serialParser.on('callStatus', (callStatus) =>{
            this.#eventEmitter.emit('callStatus', callStatus);
        ...
        this.#serialParser.on('CMGS', ({messageReference, sdsStatus}) => {
            var message = this.#sdsMessages.find(msg => msg.messageReference == messageReference);
            if (message) { //sdsStatus 4 = sent, 5 - failed
                ...
                if(message.deliveryReport != true && message.consumedReport != true) {
                    this.#waitingForMessageSentAck = false;
                    message.messageReference = null; //clear message reference
                }
        ...
        this.#serialParser.on('SDSTRANSFER-WITH-METADATA', ({sdsTransfer, callingParty}) => {
            ...
            if(sdsTransfer.protocolIdentifier == 130 || sdsTransfer.protocolIdentifier == 137){
                ...
                body[0] = Buffer.from(body[0]).toString('hex');

                const isPartOfMultiMessageResponse = body[0].includes('efbfbd');
                const isLastMessage = body[0].includes('efbfbd') == false && parseInt(body[0]) >= 0 && parseInt(body[0]) < 15;
                body.shift();

                if(!this.#partialSdsResponseBuffer[sdsTransfer.issi]) this.#partialSdsResponseBuffer[sdsTransfer.issi] = [];

                if(isPartOfMultiMessageResponse){
                    this.#partialSdsResponseBuffer[sdsTransfer.issi].push(body);

                    this.#eventEmitter.emit('partial-remote-control-response', {
                        issi: sdsTransfer.issi,
                        messageReference: sdsTransfer.messageReference,
                        body: body
                    });
                } else if(isLastMessage){
                    if(this.#partialSdsResponseBuffer[sdsTransfer.issi].length > 0){
                        this.#partialSdsResponseBuffer[sdsTransfer.issi].push(body);
                        body = this.#partialSdsResponseBuffer[sdsTransfer.issi].flat();
                        this.#partialSdsResponseBuffer[sdsTransfer.issi] = [];
                    }
        ...
        setInterval(() => {
            ...
            if (this.callStatus.active == false && pendingMessages.length > 0 && timeSinceLastMessage > 1000 && (this.#waitingForMessageSentAck == false || timeSinceLastMessage > 5000)) {
