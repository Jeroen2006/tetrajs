...
class SerialParser {
    ...
    #parseData(data) {
        ...
        const oneLineCommands = ['+CMGS', '+CTBCT', '+CSQ', '+CNUMF', '+CTOM', '+CCLK', '+CTOCP', '+CTICN', '+CTXG', '+CDTXC', '+CTCR'];
    ...
    #parseCommand(lineOne, lineTwo){
        ...
        switch(command){
            case '+CTICN':
                var [ccInstance, callStatus, aiService, callingPartyType, callingParty, hook, isSimplex, endToEndEncryption, commsType, slotCodec, calledPartyType, calledParty, priority] = value.split(",");
                this.#currentCall.active = true;


                if(callingParty != ''){
                    const callingPartyMCC = parseInt(callingParty.substring(0, 3));
                    const callingPartyMNC = parseInt(callingParty.substring(3, 7));
                    const callingPartySSI = parseInt(callingParty.substring(7));
                ...
                if(calledParty != ''){
                    const calledPartyMCC = parseInt(calledParty.substring(0, 3));
                    const calledPartyMNC = parseInt(calledParty.substring(3, 7));
                    const calledPartySSI = parseInt(calledParty.substring(7));
                ...
                this.#eventEmitter.emit('callStatus', this.#currentCall);
                break;
            case '+CTXG':
                var [ccInstance, txGrant, txRqPrmsn, endToEndEncryptionm, tpiType, tpi] = value.split(",");
                this.#currentCall.active = true;

                if(tpi != '' && tpi != null){
                    const callingPartyMCC = parseInt(tpi.substring(0, 3));
                    const callingPartyMNC = parseInt(tpi.substring(3, 7));
                    const callingPartySSI = parseInt(tpi.substring(7));
                ...
                break;
            case '+CDTXC': //TG Idle
                var [ccInstance, txRqPrmsn] = value.split(",");
                this.#currentCall.active = true;
                ...
                this.#eventEmitter.emit('callStatus', this.#currentCall);
                break;
            case '+CTCR': //TG Release
                var [ccInstance, disconnectCause] = value.split(",");
                this.#currentCall.active = false;
            ...
            case '+CTSDSR':
                ...
                if(service == '13'){ //status message
                    const statusCode = parseInt(lineTwo, 16);
